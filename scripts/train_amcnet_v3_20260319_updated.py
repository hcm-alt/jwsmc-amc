...
def load_amc_data_v3(data_path="/home/mywsl/jwsmc/data"):
    ...
    # 【关键】权重提升到3.0
    sample_weights = np.where(train_snr_tags < -6, 3.0, 1.0)
    
    print(f"\n数据分布:")
    print(f"训练集: {x_train.shape}")
    print(f"样本权重: 低SNR=3.0, 高SNR=1.0")
...
def train_amcnet_v3():
    ...
    callbacks = [
        ReduceLROnPlateau(monitor='val_accuracy', factor=0.5, patience=10, min_lr=1e-6, verbose=1),
